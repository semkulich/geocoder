...
use Geocoder\Provider\GooglePlace;
...
class GooglePlaceTest extends TestCase
{
    ...
    {
        $provider = new GooglePlace($this->getMockAdapter($this->never()));
    ...
    {
        $provider = new GooglePlace($this->getMockAdapter());
     ...
     * @expectedExceptionMessage The GooglePlace provider does not support IP addresses, only street addresses.
     */
    ...
    {
        $provider = new GooglePlace($this->getAdapter());
    ...
    {
        $provider = new GooglePlace($this->getMockAdapterReturns(null));
    ...
    {
        $provider = new GooglePlace($this->getMockAdapterReturns('{"status":"OK"}'));
        $provider->geocode('10 avenue Gambetta, Paris, France');
    }
    ...
    {
        $provider = new GooglePlace($this->getMockAdapterReturns('{"status":"OVER_QUERY_LIMIT"}'));
        $provider->geocode('10 avenue Gambetta, Paris, France');
    }

    public function testGeocodeWithRealAddress()
    {
        $provider = new GooglePlace($this->getAdapter(), 'fr-FR', 'Île-de-France');
    ...
    {
        $provider = new GooglePlace($this->getAdapter(), null, null, true);
        $results  = $provider->geocode('10 avenue Gambetta, Paris, France');
    ...
    {
        $provider = new GooglePlace($this->getMockAdapterReturns('{"error_message":"The provided API key is invalid.", "status":"REQUEST_DENIED"}'));
        $provider->geocode('10 avenue Gambetta, Paris, France');
    }
    ...
    public function testGeocodeWithRealValidApiKey()
    {
        ...
        $provider = new GooglePlace($this->getAdapter($_SERVER['GOOGLE_GEOCODING_KEY']), null, null, true, $_SERVER['GOOGLE_GEOCODING_KEY']);
    ...
    {
        $provider = new GooglePlace($this->getAdapter(), null, null, true, $this->testAPIKey);
